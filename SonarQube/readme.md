...
```
### create pipeline using s3 bucket
---
pipeline {
    agent any 
    tools{
    ...
    SCANNER_HOME = tool 'sonar-scanner'
     ...
     S3_BUCKET = "project-insure-me-build-artifacts-store-onb36"
     REGION = "ap-south-1"
    ...
    stages {
       ...
       stage('docker-image'){
            steps{
                sh 'docker build -t khanimran83823/insure .'
        ...
        stage('image-push'){
            steps {
                ...
                sh 'docker push khanimran83823/insure'
               }
            }
        } 
        ...
        stage('code-deploy'){
            steps{
                sh 'docker run -itd --name insure-me -p 8089:8081 khanimran83823/insure'
            }
        }
    }
}
